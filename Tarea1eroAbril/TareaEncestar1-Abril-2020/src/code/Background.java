...
public class Background {
    ...
    public Background(int width, int height, Game game) {
        ...
        this.animationPause = new Animation(Assets.backgroundPause, 100);
    }
    ...
    public void tick(int status) {
        if (status == 1) {
            ...
            this.animationPause.tick(); // tick animation of paused game
        }
    }
    
    public void render(Graphics g, int status) {
        if (status == 1) { // game is running
